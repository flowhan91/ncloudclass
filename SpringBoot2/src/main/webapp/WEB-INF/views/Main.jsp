...
<body>
	...
	<div id="wrapper">
		...
		<!-- Q6. 로그인을 한 상태에서는 로그인탭 대신 로그아웃탭과 개인정보수정탭을 출력 --> <!-- 로그인 후>개인정보 수정/로그아웃 메뉴 띄우기 -->
		<c:if test="${empty member }">
			<a href="#menu">로그인</a>
		</c:if> <c:if test="${not empty member }">
			<c:if test="${member.email eq 'admin'}">
				<a href="showMember">회원정보 관리</a>
			</c:if>
			<c:if test="${member.email ne 'admin'}">
				<a href="goUpdate">개인정보 수정</a>
			</c:if>
			<a href="logout">로그아웃</a>
		</c:if> <!-- Q7. 개인정보수정 기능 만들기 --> <!-- Q8. 로그아웃 기능 만들기 --> <!-- Q9. 관리자 계정(admin)일 때는 회원정보관리 탭 만들기 -->
		</nav> </header>
		...
		<div class="inner">
			...
			<ul class="actions vertical">
				...
				<form action="join" method="post">
					<li><input type="text" name="email" id="inputEmail"
						placeholder="Email을 입력하세요"></li>
					<li><input type="button" value="Emai 중복체크"
		...
		<div class="inner">
			...
			<!-- ex) smhrd님 환영합니다.  --> <!-- 로그인 수행 전/후에 따른 h1태그 출력 --> <!--empty: null과 같은 의미 -->
			<c:if test="${empty member }">
				<h1>로그인 한 세션아이디를 출력해주세요</h1>
			</c:if> <!-- 로그인 후>>Session에 member라는 데이터가 있다면 --> <c:if
				test="${not empty member }">
				<h1>${member.email }님환영합니다.</h1>
			</c:if> </header>
			<div class="content">
				<p>
					게시판을 이용해보세요 ^^<br>
		...
		<div id="main">
			...
			<section id="one" class="tiles"> <article> <span
				class="image"> <img src="/web/images/pic01.jpg" alt="" />
	...
	<!-- 비동기 통신으로 회원가입의 이메일 여부를 체크할 수 있는 기능 만들기 -->
	<script type="text/javascript">
				//ajax: JS 비동기 통신을 위해 사용하는 기술이다. 
				//1. 입력된 이메일 가져오기
				let email = $('#inputEmail').val();
				
				$.ajax(
					url : "checkEmail",	 //controller가 받는 요청값
					type: "get",
					data: {inputEmail: email},
					success: function(res){ 
						//res : controller의 작업 이후 결과값(중복 여부) -> String "true" or "false"
						if(res=="true"){
							$("#resultEmail").text("사용가능한 이메일 입니다.").css("color","green")); 
						}else{
